                            ...
                            @foreach($tasks as $task)
                                @if($user['id']==$task['user_id'])
                                    @if($task['status']==1)
                                        ...
                                        @else
                                       <a href="{{route('tasks.edit',$task->id)}}"><div title="{{$task['title']}}" style="letter-spacing: .00625em;font-weight: 400;font-size: 1rem;font-family: Roboto,Arial,sans-serif;background:{{$colors[$task->color_id]}}; width:{{$times[$task->time_id]*12.5}}%;position:absolute; left:{{$task->start_time_id * 6.25+12.5}}%;display: inline-block;height: 25px;margin-top: 5px;margin-bottom: 5px;text-align:center;color:#202138;">
                                    @endif
                                           {{$task['title']}}</div></a>
                                    @endif
                            @endforeach
